...
export const custArray: number[] = [5, 50, 100, 200];

const sumArray = custArray.reduce((total, amt) => {
  console.log(total + '-' + amt);
  return total + amt;
});

const avgarr = custArray.reduce((total, amt, index, array) => {
  console.log(total + '-' + amt);
  total += amt;
...
const count = fruitBasket.reduce((tally: any, fruit) => {
...
const removeApple = fruitBasket.reduce((tally: any, fruit) => {
  if (fruit == 'apple') {
    return tally;
  } else {
    tally[fruit] = (tally[fruit] || 0) + 1;
